...
export async function deployContractToZKNet(hre: HardhatRuntimeEnvironment, config: any): Promise<ethers.Contract> {
    ...
    let gasConfig = {};
    if (config.GasPrice) {
        gasConfig = Object.assign(gasConfig, { gasPrice: BigInt(config.GasPrice) });
        gasConfig = Object.assign(gasConfig, { gasLimit: BigInt(config.DeployGasLimit) });
    }
    const contract = await deployer.deploy(artifact, [], undefined, gasConfig);
...
export async function deployContract(hre: any, config: any): Promise<ethers.BaseContract> {
    const { ethers } = hre;

...
OwnableUpgradeable {
    ...
    function mint_(address _to, string calldata _uri, bytes calldata _data, uint _fee, uint256 tokenId) internal returns (uint256) {
    ...
    function mint(address _to, string calldata _uri, bytes calldata _data, uint _fee) external payable onlyManager returns (uint256) {
        while (datas[nextTokenId].data.length != 0) {
            nextTokenId++;
        }
    ...
    function getHashToSign(
        ...
        bytes32 structHash = keccak256(abi.encode(_to, _uri, _data, _fee, _manager));
    ...
    function updateTokenData(uint256 _tokenId, bytes calldata _data, uint _fee) external onlyOwner {
        require(_data.length != 0, "invalid data input");

        if (_data.length != 0) {
            datas[_tokenId].data = _data;
        }
    ...
    function infer(uint _tokenId, bytes calldata _calldata) external payable {
        ...
        require(data.data.length != 0, "inc");
        require(msg.value >= data.fee, "ivf");

        fees[_ownerOf(_tokenId)] += data.fee;
        bytes memory fwdData = abi.encodePacked(data.data, _calldata);
    ...
    function dataOf(uint256 _tokenId) external view returns (TokenMetaData memory) {

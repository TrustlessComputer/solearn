...
library Tensors {
	...
	function from(Tensor2D memory ts, SD59x18[][] memory mat) internal pure returns (Tensor2D memory) {
		ts.n = mat.length;
		ts.m = mat[0].length;
		ts.mat = mat;
		return ts;
	...
	function load(Tensor2D memory ts, SD59x18[] memory data, uint n, uint m) internal pure {
		ts.n = n;
		ts.m = m;
	...
	function cloneTensor(Tensor2D memory ts) internal pure returns (Tensor2D memory) {
		Tensor2D memory result;
	...
	) internal pure returns (Tensor2D memory) {
		Tensor2D memory res = cloneTensor(a);
	...
	function activation(Tensor2D memory a, ActivationFunc actv) internal pure returns (Tensor2D memory) {
		if (actv == ActivationFunc.LeakyReLU) {
			return __apply_unary_op(a, Function.__leaky_relu);
		} else if (actv == ActivationFunc.Linear) {
			return __apply_unary_op(a, Function.__linear);
		} else if (actv == ActivationFunc.ReLU) {
			return __apply_unary_op(a, Function.__relu);
		} else if (actv == ActivationFunc.Sigmoid) {
			return __apply_unary_op(a, Function.__sigmoid);
		} else if (actv == ActivationFunc.Tanh) {
			return __apply_unary_op(a, Function.__tanh);
		} else {
			revert InvalidActivationFunction();
		}
  	}

    function __apply_binary_op(Tensor2D memory a, Tensor2D memory b, function(SD59x18, SD59x18) internal pure returns (SD59x18) op) internal pure returns (Tensor2D memory) {
		if (b.n == 0) {
			b.n = 1;
			b.m = 1;
			b.mat = new SD59x18[][](1);
    ...
    function mul(Tensor2D memory a, Tensor2D memory b) internal pure returns (Tensor2D memory) {
		return __apply_binary_op(a, b, Function.__mul);
	}
	
    function add(Tensor2D memory a, Tensor2D memory b) internal pure returns (Tensor2D memory) {
    	return __apply_binary_op(a, b, Function.__add);
    }

    function matMul(Tensor2D memory a, Tensor2D memory b) internal pure returns (Tensor2D memory) {
    	Tensor2D memory res;
    	res.n = a.n;
    	res.m = b.m;
    ...
    function softmax(Tensor2D memory a) internal pure returns (Tensor2D memory) {
		Tensor2D memory res = __apply_unary_op(a, Function.__exp);
		SD59x18 sum_e = sd(0);
		for (uint i = 0; i < res.n; i++) {
			for (uint j = 0; j < res.m; j++) {
